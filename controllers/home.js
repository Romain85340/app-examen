module.exports = {
    ...
    homePage : async (req, res) => {
        try {
            ...
            const posts = await query("SELECT i.id, c.title AS category, c.id AS id_category , u.firstname, u.lastname, i.title, i.content, i.image, i.date, ifnull(count(s.bad), 0) as bad_status, ifnull(count(s.good), 0) AS good_status FROM item AS i INNER JOIN user AS u ON u.id = i.id_user INNER JOIN category AS c ON c.id = i.id_category LEFT OUTER JOIN status AS s ON s.id_item = i.id GROUP BY i.id ORDER BY i.date DESC LIMIT 3")
    ...
    listBySport : async (req, res) => {
        ...
        try {
            const posts = await query("SELECT i.id AS id_post, c.id AS id_category, c.title AS category , u.firstname, u.lastname, i.title, i.content, i.image, i.date, ifnull(count(s.bad), 0) as bad_status, ifnull(count(s.good), 0) AS good_status FROM item AS i INNER JOIN user AS u ON u.id = i.id_user INNER JOIN category AS c ON c.id = i.id_category LEFT OUTER JOIN status AS s ON s.id_item = i.id WHERE c.id = ? GROUP BY i.id ORDER BY i.date DESC", [id])
    ...
    onePost: async (req, res) => {
        ...
        try {
            const posts = await query("SELECT i.id AS id_post, c.title AS category, c.id AS id_category , u.firstname, u.lastname, i.title, i.content, i.image, i.date, ifnull(count(s.bad), 0) as bad_status, ifnull(count(s.good), 0) AS good_status FROM item AS i INNER JOIN user AS u ON u.id = i.id_user INNER JOIN category AS c ON c.id = i.id_category LEFT OUTER JOIN status AS s ON s.id_item = i.id WHERE c.id = ? AND i.id = ? GROUP BY i.id ORDER BY i.date DESC", [category, post])
            ...
            res.render("one-post", {post: posts[0], bad_status, good_status})
        } catch(err){
            res.send(err)
        }
    }
